  ...
  public async run(action: IActionRdfUpdateQuadsInfo): Promise<IActorRdfUpdateQuadsInfoOutput> {
    return {
      execute: async() => {
        ...
        const quadStreamInsert = (await (await this.mediatorRdfFilterExistingQuads.mediate({
          filterSource: action.filterSource && context !== false,
          filterDestination: true,
          quadStream: action.quadStreamInsert,
