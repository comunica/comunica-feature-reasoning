  ...
  beforeEach(() => {
    ...
    mediatorRdfUpdateQuadsInfo = {
      async mediate(action: IActionRdfUpdateQuadsInfo): Promise<IActorRdfUpdateQuadsInfoOutput> {
        return {
          async execute() {
            const store = action.context.get(KeysRdfUpdateQuads.destination) as Store;
    ...
    mediatorFilterExistingQuads = {
      async mediate(action) {
        return {
          async execute() {
            const store = action.context.get(KeysRdfResolveQuadPattern.source) as Store;
