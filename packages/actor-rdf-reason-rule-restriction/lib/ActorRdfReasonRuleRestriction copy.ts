...
import type { IActionRdfReason, IActionRdfReasonExecute, IActorRdfReasonMediatedArgs, IActorRdfReasonOutput } from '@comunica/bus-rdf-reason';
...
export class ActorRdfReasonRuleRestriction extends ActorRdfReasonMediated {
  ...
  public async execute(action: IActionRdfReasonExecute): Promise<void> {
    ...
    let size = 0;
    const rules = await action.rules.toArray();
