...
import { ArrayIterator, fromArray, UnionIterator, wrap } from 'asynciterator';
...
function maybeSubstitute({ rule: { rule, next }, index }: { rule: IRuleNode; index: number }, quad: RDF.Quad):
IRuleNode | null {
...
export class ActorRdfReasonForwardChaining extends ActorRdfReasonMediated {
  public readonly mediatorRuleEvaluate: MediatorRuleEvaluate;
  ...
  private evaluateInsertRule(rule: IRuleNode, context: IActionContext): IConsequenceData {
    const quads: AsyncIterator<RDF.Quad> = wrap(this.evaluateInsert(rule, context));
    return { quads, rule };
  }
