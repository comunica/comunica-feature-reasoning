...
function load(filename: string, store: Store) {
  return new Promise<void>((res) => {
    new Parser({ baseIRI: 'http://example.org' }).parse(fs.createReadStream(path.join(__dirname, filename)), (error: Error, quad: RDF.Quad) => {
      assert(!error, error);
      if (quad)
        store.add(quad);
      else {
...
async function run(name: string, rule: string, data: string[]) {
