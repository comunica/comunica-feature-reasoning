...
import { ActorRuleDereferenceHttpParse } from '../lib/ActorRuleDereferenceHttpParse';
const arrayifyStream = require('arrayify-stream');
const streamifyString = require('streamify-string');

describe('ActorRuleDereferenceHttpParse', () => {
  let bus: any;
  ...
  beforeEach(() => {
    ...
    mediatorRuleParse = new MediatorRace({ name: 'mediator-parse', bus: new Bus({ name: 'bus-parse' }) });
    mediaMappings = {
      x: 'y',
    };
  });

  describe('The ActorRuleDereferenceHttpParse module', () => {
    it('should be a function', () => {
      expect(ActorRuleDereferenceHttpParse).toBeInstanceOf(Function);
    });

    it('should be a ActorRuleDereferenceHttpParse constructor', () => {
      expect(new (<any> ActorRuleDereferenceHttpParse)(
        { name: 'actor', bus, mediatorHttp, mediatorRuleParse, mediaMappings },
      ))
        .toBeInstanceOf(ActorRuleDereferenceHttpParse);
      expect(new (<any> ActorRuleDereferenceHttpParse)(
        { name: 'actor', bus, mediatorHttp, mediatorRuleParse, mediaMappings },
      ))
        .toBeInstanceOf(ActorRuleDereference);
    });

    it('should not be able to create new ActorRuleDereferenceHttpParse objects without \'new\'', () => {
      expect(() => { (<any> ActorRuleDereferenceHttpParse)(); }).toThrow();
    });
  });

  describe('An ActorRuleDereferenceHttpParse instance', () => {
    let actor: ActorRuleDereferenceHttpParse;

    beforeEach(() => {
      mediatorRuleParse.mediate = jest.fn((action: any) => {
        ...
        if (action.context && action.context.has('emitParseError')) {
          ...
          return { handle: { quads: rules, triples: true }};
        ...
        action.handle.input.on('error', (error: Error) => rules.emit('error', error));
        return { handle: { rules }};
      });
      ...
      actor = new ActorRuleDereferenceHttpParse({
        bus,
        maxAcceptHeaderLength: 127,
        maxAcceptHeaderLengthBrowser: 127,
        mediaMappings,
        mediatorHttp,
        mediatorRuleParseHandle: mediatorRuleParse,
        mediatorRuleParseMediatypes: mediatorRuleParse,
        name: 'actor',
      });
    });
    ...
    it('should run with an empty content type', async() => {
      ...
      expect(output.headers).toEqual(headers);
      expect(await arrayifyStream(output.rules)).toEqual([]);
      expect(mediatorRuleParse.mediate).toHaveBeenCalledWith({
    ...
    it('should run and receive stream errors', async() => {
      ...
      await expect(arrayifyStream(output.rules)).rejects.toThrow(new Error('Body stream error'));
    });
